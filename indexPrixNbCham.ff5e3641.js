...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"scripts/explCoutNbCham/helper.js":[function(require,module,exports) {
"use strict";
...
function generateG(margin) {
  return d3.select('.graph-prix-nbCham').select('svg').append('g').attr('id', 'graph-g-nbCham').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
}
...
function appendAxes(g) {
  g.append('g').attr('class', 'x axis-nbCham');
  g.append('g').attr('class', 'y axis-nbCham');
}
/**
...
function appendGraphLabels(g) {
  g.append('text').text('Prix ($)').attr('class', 'y axis-text nbCham').attr('transform', 'rotate(-90)').attr('fill', '#898989').attr('font-size', 12);
  g.append('text').text("Nombre de chambres").attr('class', 'x axis-text nbCham').attr('fill', '#898989').attr('font-size', 12);
  g.append('text').text('Variation du prix en fonction du nombre de chambre').attr('class', 'title nbCham').attr('fill', '#898989');
}
...
function setCanvasSize(width, height) {
  d3.select('#bar-chart-prix-nbCham').select('svg').attr('width', width).attr('height', height);
}
...
function positionLabels(width, height) {
  d3.select('.y.axis-text.nbCham').attr('x', -70).attr('y', height / 2);
  d3.select('.x.axis-text.nbCham').attr('x', width / 2).attr('y', height + 50);
  d3.select('.title.nbCham').attr('x', width / 2).attr('y', -35);
}
...
function drawXAxis(xScale, height) {
  d3.select('.x.axis-nbCham').attr('transform', 'translate(0, ' + height + ')').call(d3.axisBottom(xScale));
}
...
},{}],"scripts/explCoutNbCham/preprocess.js":[function(require,module,exports) {
"use strict";
...
function clean_data(data) {
  return data.filter(function (elem) {
    return elem["nbBeds"] != 0 && elem["nbBeds"] != "Not Available";
  });
}
...
},{}],"scripts/explCoutNbCham/viz.js":[function(require,module,exports) {
"use strict";
...
function drawBoxes(y, x, data, height) {
  ...
  var line1 = d3.select('#graph-g-nbCham').selectAll('.area-circle').data(keys).enter().append('path');
  line1.attr('d', function (elem) {
    return d3.line()([[x(elem) + left(width), y(data[elem]["10"])], [x(elem) + right(width), y(data[elem]["10"])]]);
  }).attr('class', 'nbChamCircles').attr('stroke', 'black');

  //draw the 90th percentile
  var line2 = d3.select('#graph-g-nbCham').selectAll('.area-circle').data(keys).enter().append('path');
  line2.attr('d', function (elem) {
    return d3.line()([[x(elem) + left(width), y(data[elem]["90"])], [x(elem) + right(width), y(data[elem]["90"])]]);
  }).attr('class', 'nbChamCircles').attr('stroke', 'black');

  //draw the middle line
  var line3 = d3.select('#graph-g-nbCham').selectAll('.area-circle').data(keys).enter().append('path');
  line3.attr('d', function (elem) {
    return d3.line()([[x(elem) + width / 2, y(data[elem]["10"])], [x(elem) + width / 2, y(data[elem]["90"])]]);
  }).attr('class', 'nbChamCircles').attr('stroke', 'black');

  //draw the rectangle
  var rect = d3.select('#graph-g-nbCham').selectAll('.area-circle').data(keys).enter().append('rect');
  rect.attr('x', function (elem) {
    return x(elem) + left(width);
  }).attr('y', function (elem) {
    return y(data[elem]["75"]);
  }).attr('class', 'nbChamCircles').attr('width', right(width) - left(width)).attr('height', function (elem) {
    return y(data[elem]["25"]) - y(data[elem]["75"]);
  }).attr('stroke', 'black').attr('fill', '#69a3b2');

  //draw the median
  var median = d3.select('#graph-g-nbCham').selectAll('.area-circle').data(keys).enter().append('path');
  median.attr('d', function (elem) {
    return d3.line()([[x(elem) + left(width), y(data[elem]["50"])], [x(elem) + right(width), y(data[elem]["50"])]]);
  }).attr('class', 'nbChamCircles').attr('stroke', 'black');
}
},{}],"indexPrixNbCham.js":[function(require,module,exports) {
"use strict";

var helper = _interopRequireWildcard(require("./scripts/explCoutNbCham/helper"));
var preproc = _interopRequireWildcard(require("./scripts/explCoutNbCham/preprocess"));
var viz = _interopRequireWildcard(require("./scripts/explCoutNbCham/viz"));
...
},{"./scripts/explCoutNbCham/helper":"scripts/explCoutNbCham/helper.js","./scripts/explCoutNbCham/preprocess":"scripts/explCoutNbCham/preprocess.js","./scripts/explCoutNbCham/viz":"scripts/explCoutNbCham/viz.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","indexPrixNbCham.js"], null)
//# sourceMappingURL=/indexPrixNbCham.ff5e3641.js.map