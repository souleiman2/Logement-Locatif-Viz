 ...
 */
 export function generateG (margin) {
  return d3.select('.graph-prix-type')
    .select('svg')
    .append('g')
    .attr('id', 'graph-g-type')
    .attr('transform',
...
export function appendGraphLabels (g) {
  ...
  g.append('text')
    ...
    .attr('class', 'x axis-text type')
    .attr('fill', '#898989')
    .attr('font-size', 12)

  g.append('text')
    .text("Prix en fonction du type d'habitation")
    .attr('class', 'title type')
    .attr('fill', '#898989')
}
...
export function setCanvasSize (width, height) {
  d3.select('#bar-chart-prix-type').select('svg')
    .attr('width', width)
    .attr('height', height)
}
...
export function positionLabels (width, height) {
  d3.select('.y.axis-text.type')
    .attr('x', -70)
    .attr('y', height / 2)

  d3.select('.x.axis-text.type')
    .attr('x', width / 2)
    .attr('y', height + 50)

  d3.select('.title.type')
    .attr('x', width / 2)
    .attr('y', -35)
}
