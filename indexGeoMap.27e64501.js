...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"scripts/geoMap/helper.js":[function(require,module,exports) {
"use strict";
...
},{}],"assets/data/map_code_Postaux.json":[function(require,module,exports) {
module.exports = {
  "Rivière-des-Prairies-Pointe-aux-Trembles": ["H1A"],
  "Villeray-Saint-Michel-Parc-Extension": ["H2A", "H2E", "H3N", "H2P", "H2R", "H1Z"],
  "Côte-des-Neiges-Notre-Dame-de-Grâce": ["H4A", "H4B", "H3S", "H3T", "H3V", "H3W"],
  "Dollard-Des-Ormeaux": ["H9A", "H9B", "H9G"],
  "Montréal-Est": ["H1B"],
  "Ahuntsic-Cartierville": ["H2B", "H2C", "H4J", "H4K", "H3L", "H2M", "H3M", "H2N"],
  "L'Île-Bizard-Sainte-Geneviève": ["H9C", "H9H"],
  "Montréal-Nord": ["H1G", "H1H"],
  "Rosemont-La Petite-Patrie": ["H2G", "H2S", "H1X", "H1Y"],
  "Verdun": ["H4G", "H4H"],
  "Le Plateau-Mont-Royal": ["H2H", "H2J", "H2T", "H2W", "H2X"],
  "Anjou": ["H1J", "H1K"],
  "Kirkland": ["H9J"],
  "Senneville": ["H9K"],
  "Mercier-Hochelaga-Maisonneuve": ["H1L", "H1M", "H1N", "H1V", "H1W"],
  "Saint-Laurent": ["H4L", "H4M", "H4N", "H4R", "H4S", "H4T"],
  "LaSalle": ["H8N", "H8P", "H4E"],
  "Ville-Marie": ["H3A", "H3B", "H3G", "H3H", "H2Y", "H2Z", "H4Z"],
  "Saint-Léonard": ["H1P", "H1R", "H1S"],
  "Mont-Royal": ["H3P", "H3R", "H4P"],
  "Dorval": ["H9P", "H4Y"],
  "Pointe-Claire": ["H9R"],
  "Lachine": ["H8S", "H8T"],
  "L'Île-Dorval": ["H9S"],
  "Outremont": ["H2V"],
  "Côte-Saint-Luc": ["H4V", "H4W"],
  "Beaconsfield": ["H9W"],
  "Hampstead": ["H3X"],
  "Montréal-Ouest": ["H4X"],
  "Westmount": ["H3Y", "H3Z"],
  "Pierrefonds-Roxboro": ["H8Y", "H8Z"]
};
},{}],"scripts/geoMap/preprocess.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.MapNeighborhood = MapNeighborhood;
      ...
      for (var index = 0; index < neighborhoods[neighborhood].length; index++) {
        if (annonce.location.toUpperCase().includes(neighborhoods[neighborhood][index].toUpperCase())) {
          if (!(neighborhood in obj)) {
            ...
            var tempPrice = parseInt(annonce.price),
            ...
            obj[neighborhood]["total"] += 1;
            var _tempPrice = parseInt(annonce.price),
              _tempSuperficie = parseInt(annonce.areainfeet);
            if (_tempPrice) {
              temp[neighborhood][0] += 1;
            }
            if (_tempSuperficie) {
            ...
            obj[neighborhood]["prix"] += _tempPrice ? _tempPrice : 0;
            obj[neighborhood]["superficie"] += _tempSuperficie ? _tempSuperficie : 0;
          }
          break;
        }
      }
...
},{"../../assets/data/map_code_Postaux.json":"assets/data/map_code_Postaux.json"}],"scripts/geoMap/viz.js":[function(require,module,exports) {
"use strict";
 ...
 */
function mapBackground(data, path) {
  d3.select("#map-g").selectAll("path").data(data["features"]).enter().append("path").attr("class", "paths").attr("d", function (elem) {
    return path(elem);
  }).style("fill", "lightgrey").attr("stroke", "white").attr("stroke-width", "1px");
}
...
function setRadiusScale(scale, data) {
  var keys = Object.keys(data);
  var superficie = keys.map(function (key) {
    return data[key]["superficie"];
  });
  scale.domain([Math.min.apply(Math, _toConsumableArray(superficie)), Math.max.apply(Math, _toConsumableArray(superficie))]).range([2, 12]);
}
 ...
 */
function setColorScale(scale, data, colors) {
  var keys = Object.keys(data);
  var prices = keys.map(function (key) {
    return data[key]["prix"];
  });
  scale.domain(_toConsumableArray(prices), Math.max.apply(Math, _toConsumableArray(prices))).range(colors); //.interpolate(d3.interpolateHcl)
}
 ...
 * @param {*} tip Tooltip
 */
function mapMarkers(radiusScale, colorScale, procData, districts, path, tip) {
  districts = districts.filter(function (elem) {
    return procData.hasOwnProperty(elem["properties"]["NOM"]);
  });
  var circles = d3.select("#marker-g").selectAll("circle").data(districts).enter().append("circle").attr("r", function (elem) {
    return radiusScale(procData[elem["properties"]["NOM"]]["superficie"]);
  }).attr("cx", function (elem) {
    return path.centroid(elem)[0];
  }).attr("cy", function (elem) {
    return path.centroid(elem)[1];
  }).attr("fill", function (elem) {
    return colorScale(procData[elem["properties"]["NOM"]]["prix"]);
  }).attr("stroke", "white").attr("stroke-width", "1px").on('mouseover', function (d) {
    tip.show(_objectSpread(_objectSpread({}, procData[d["properties"]["NOM"]]), {}, {
      "name": d["properties"]["NOM"]
    }), this);
  }).on('mouseout', function (d) {
    tip.hide(_objectSpread(_objectSpread({}, procData[d["properties"]["NOM"]]), {}, {
      "name": d["properties"]["NOM"]
    }), this);
  });
  circles.call(tip);
}
},{}],"scripts/geoMap/legend.js":[function(require,module,exports) {
"use strict";
 ...
 */
function drawLegend(radiusScale, colorScale, g) {
  var BOTTOM_UPPER_ELEMENT = 40;
  var LEFT_UPPER_ELEMENT = 820;
  var OFFSET = 20;
  var BAR_HEIGHT = 200;

  //titles
  g.append('text').text('Légende').attr('font-weight', "bold").attr('transform', "translate(" + LEFT_UPPER_ELEMENT.toString() + "," + BOTTOM_UPPER_ELEMENT.toString() + ")");
  g.append('text').text('Prix médian').attr("font-size", "12").attr('transform', "translate(" + LEFT_UPPER_ELEMENT.toString() + "," + (BOTTOM_UPPER_ELEMENT + OFFSET * 1.5).toString() + ")");
  ...
  var svg = g.append('svg').attr('width', 30).attr('height', BAR_HEIGHT).attr('x', LEFT_UPPER_ELEMENT + 30).attr('y', (BOTTOM_UPPER_ELEMENT + OFFSET * 2).toString());
  var grad = svg.append('defs').append('linearGradient').attr('id', 'grad').attr('x1', '0%').attr('x2', '0%').attr('y1', '100%').attr('y2', '0%');
  grad.selectAll('stop').data(colors).enter().append('stop').style('stop-color', function (d) {
    return d;
  }).attr('offset', function (d, i) {
    return 100 * (i / (colors.length - 1)) + '%';
  });
  svg.append('rect').attr('x', 10).attr('y', 10).attr('width', 50).attr('height', BAR_HEIGHT).style('fill', 'url(#grad)');

  //bar axes
  var yScale = d3.scaleLinear();
  var domain = colorScale.domain();
  yScale.domain([Math.min.apply(Math, _toConsumableArray(domain)), Math.max.apply(Math, _toConsumableArray(domain))]).range([BAR_HEIGHT, 0]);
  g.append("g").attr('transform', "translate(" + (LEFT_UPPER_ELEMENT + 30).toString() + "," + (BOTTOM_UPPER_ELEMENT + OFFSET * 2).toString() + ") ").call(d3.axisLeft(yScale).ticks(5));
  var NEW_BOTTOM_UPPER_ELEMENT = BOTTOM_UPPER_ELEMENT + OFFSET * 5 + BAR_HEIGHT;
  g.append('text').text('Superficie en pieds² médianne').attr("font-size", "12").attr('transform', "translate(" + LEFT_UPPER_ELEMENT.toString() + "," + NEW_BOTTOM_UPPER_ELEMENT.toString() + ")");
  var total_offset = NEW_BOTTOM_UPPER_ELEMENT + OFFSET;
  for (var i = 0; i <= 4; i++) {
    var super_value = 1800 * (i / 4);
    var value = radiusScale(super_value);
    total_offset += value * 2 + OFFSET / 2;
    g.append('circle').attr('cx', LEFT_UPPER_ELEMENT + OFFSET).attr('cy', total_offset).attr('r', value).attr('fill', 'black');
    g.append('text').text(super_value.toString() + " pieds²").attr('font-size', 12).attr('x', LEFT_UPPER_ELEMENT + OFFSET * 2).attr('y', total_offset + value / 2).attr('fill', 'black');
  }
}
},{}],"scripts/geoMap/tooltip.js":[function(require,module,exports) {
"use strict";
...
function getContents(d) {
  ...
  */
  ...
  return "<div style='font-weight:normal'>\n            <b>Nom du quartier : </b>".concat(d["name"], "<br/>\n            <b>Prix m\xE9dian : </b>").concat(Number(Number(d["prix"]).toFixed(2)), "$<br/>\n            <b>Superficie m\xE9dianne : </b>").concat(Number(Number(d["superficie"]).toFixed(2)), " pieds\xB2<br/>\n            <b>Taille d'\xE9chantillon : </b>").concat(d["total"], "<br/>\n          </div>");
}
...
},{"d3-collection":"../node_modules/d3-collection/src/index.js","d3-selection":"../node_modules/d3-selection/src/index.js"}],"indexGeoMap.js":[function(require,module,exports) {
"use strict";

var helper = _interopRequireWildcard(require("./scripts/geoMap/helper"));
var preproc = _interopRequireWildcard(require("./scripts/geoMap/preprocess"));
var viz = _interopRequireWildcard(require("./scripts/geoMap/viz"));
var legend = _interopRequireWildcard(require("./scripts/geoMap/legend"));
var tooltip = _interopRequireWildcard(require("./scripts/geoMap/tooltip"));
...
},{"./scripts/geoMap/helper":"scripts/geoMap/helper.js","./scripts/geoMap/preprocess":"scripts/geoMap/preprocess.js","./scripts/geoMap/viz":"scripts/geoMap/viz.js","./scripts/geoMap/legend":"scripts/geoMap/legend.js","./scripts/geoMap/tooltip":"scripts/geoMap/tooltip.js","d3-tip":"../node_modules/d3-tip/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","indexGeoMap.js"], null)
//# sourceMappingURL=/indexGeoMap.27e64501.js.map